...
const RandomDiagnosisCode = () => {
    ...
    return (
        <div className="flex min-h-screen flex-col overflow-x-hidden p-16 md:items-center md:p-24">
            <button type="button"
            ...
            {randomDiagnosisCode && <h1 className="break-words text-3xl"> {`Code: ${randomDiagnosisCode.code}`} </h1>}
            {randomDiagnosisCode && <h1 className="break-words text-3xl"> {`Text: ${randomDiagnosisCode.text}`} </h1>}
        </div>
    );
};
